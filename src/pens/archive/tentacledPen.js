// src/pens/tentacledPen.js

import { Octopus } from '../../animals/archive/octopus.js';
import { Ball, Box, Food, Log, PenWall } from '../objects/index.js';

/**
 * Represents a "tentacled" animal pen, currently home to an Octopus and enrichment objects.
 * Handles its own contained objects' update logic.
 */
export class TentacledPen {
  /**
  ...
  constructor(world, scene, animationSystem, offset) {
    ...
    // --- Creature (Octopus) ---
    this.octopus = new Octopus(world, scene, offset, animationSystem);
    this.objects.push(this.octopus);

    // --- Enrichment Objects ---
    // Placed within the pen and not overlapping the wall
    this.objects.push(
      new Ball(
        world,
        scene,
    ...
    // Log creation
    console.info(
      `[TentacledPen] Created at (${offset.x}, ${offset.y}, ${offset.z}) with Octopus, PenWall, and 4 enrichment objects.`
    );
  }
  ...
  remove() {
    ...
    console.info(`[TentacledPen] Removed all objects from pen ${this.id ?? ''}`);
  }
}
